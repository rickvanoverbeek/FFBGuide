...
export default function ConfiguratorPage() {
  ...
  useEffect(() => {
    async function load() {
      const [wb, g] = await Promise.all([
        client.fetch<Wheelbase[]>(allWheelbasesQuery),
        client.fetch<Game[]>(allGamesQuery),
      ]);
      setWheelbases(wb);
      setGames(g);
      setDataLoaded(true);
  ...
  useEffect(() => {
    ...
    async function fetchPreset() {
      setLoading(true);
      const result = await client.fetch<GameWheelbasePreset | null>(
        presetQuery,
        { wheelbaseSlug: selectedWheelbase, gameSlug: selectedGame }
      );
      setPreset(result);
      setLoading(false);
    }
